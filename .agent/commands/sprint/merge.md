...
PR이 이미 merge된 상태라면, 평가 라벨을 제거한다:

```bash
# 평가 라벨 제거
gh pr edit {PR_NUMBER} --repo dusunax/ux-lab --remove-label "eval: done"
```

**⚠️ 중요: findings 라벨은 절대 제거하지 않는다.**  
findings는 평가 이력을 기록하는 영구 메타데이터이다.

**제거할 라벨:**
- `eval: done` — 머지 기준 표시 (merge 완료 시 제거)
