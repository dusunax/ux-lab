...
파일 소유권은 이 문서에 복제하지 않는다. 브리핑 전 반드시 아래 SSOT를 읽고, 대상 역할의 현재 `allow`, `deny`, `readonly`, `boundary` 값을 그대로 반영한다.

- 사람이 읽는 기준: `.agent/rules/agent-scope.md`
- 기계 판독 기준: `.agent/rules/agent-scope.json`

브리핑의 `파일 범위 제약` 항목에는 `agent-scope.json`의 대상 역할 `allow` 목록을 그대로 적는다.
`readonly: true`인 역할은 항상 `읽기 전용. 파일 수정 금지. 결과는 텍스트 출력만.`으로 적는다.
경계 파일 판단은 `boundary`와 `boundary_meta`를 따른다.

권한 정책을 바꿔야 하면 `.agent/rules/agent-scope.json`만 수정하고, `.agent/scripts/gen-scope-doc.py`로 `.agent/rules/agent-scope.md`를 재생성한다.
