...
export function solarToLunar(date: Date | number): LunarDate {
  ...
  const offset = Math.floor((targetDate.getTime() - BASE_DATE.getTime()) / (24 * 60 * 60 * 1000));
...
export function getGanZhi(year: number): {stem: string; branch: string; zodiac: string; ganZhi: string} {
