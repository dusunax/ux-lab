...
| Day | 작업 | 내용 | 검증 |
|---|---|---|---|
| Day 1 | R8 + 리소스 축소 활성화 | `enableProguardInReleaseBuilds = true`, `shrinkResources true` 추가 | 릴리즈 빌드 성공 + 에뮬레이터(Medium_Phone_API_35)에 설치해 다크모드·언어전환·날짜변환 스모크 테스트 |
| Day 2 | Day 1 후속 조치 | 스모크 테스트에서 깨진 부분이 있으면 `proguard-rules.pro`에 keep 규칙 추가 | 재스모크 테스트 통과 시 0.8.3 |
