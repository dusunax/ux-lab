    ...
    <div className="screen start">
      <div className="start__sky" aria-hidden="true">
        {FLOATERS.map(([name, left, top, delay, size]) => (
          <img key={name} className="floater" src={spriteUrl(name)} alt="" width={size} style={{ left: `${left}%`, top: `${top}%`, animationDelay: `${delay}s` }} />
        ))}
      </div>
      ...
      <div className="hero-cat">
        <PullCat>
          <img src={spriteUrl('cat-back')} alt="창밖을 바라보는 고양이" width={107} height={130} draggable={false} />
        </PullCat>
      </div>
