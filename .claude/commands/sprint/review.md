...
1. `gh label list --repo [REPO]`로 `sprint/N` 라벨 존재 여부 확인
2. 없으면 생성: `gh label create "sprint/N" --color 0EA5E9 --repo [REPO]`
3. PR에 부착

**라벨 부착 방법 (우선순위):**

- `mcp__github__update_pull_request` (labels 파라미터) 우선 시도
- 미연결 시: `gh pr edit [NUMBER] --add-label "type: sprint,sprint/N,eval: pending" --repo [REPO]`
...
완료율:     M/T개 ([완료율]%)
보고서:     [URL 또는 로컬 경로]
라벨:       type: sprint, sprint/N, eval: pending
```
