...
- [ ] `/agent/new`에서 새 Agent를 작성하고 제출할 수 있다
- [ ] 등록 폼에서 Platform, Usage Guide, Visibility, Prompt, resultPreset, downloads 필드를 입력하거나 생성할 수 있다
- [ ] `POST /api/agents`가 유효한 Agent payload를 받아 mock repository에 저장한다
- [ ] 등록 완료 후 Home Feed에 새 Agent가 표시되고 Detail 화면으로 이동할 수 있다
- [ ] 생성된 Agent의 Detail과 Run 화면에서 작성자 결과 프리셋과 다운로드 파일이 기존 UI로 표시된다
- [ ] `/profile/[userId]` 또는 `/me`에서 작성 Agent, 써봤어요, 좋아요, Fork 활동 요약을 볼 수 있다
- [ ] `/requests`에서 Agent 요청 목록을 보고 새 요청을 만들 수 있다
- [ ] Request Board에서 요청 투표와 상태 badge가 local/repository state로 동작한다
- [ ] Auth shell이 로그인 필요 상태와 현재 사용자 정보를 일관되게 제공한다
- [ ] Firestore 전환 문서에 컬렉션 구조, document shape, index 후보, 보안 규칙 초안이 포함된다
- [ ] comments, likes, tried, fork 동작이 직접 local state가 아니라 repository action을 경유한다
- [ ] 빈 상태, 검색 결과 없음, validation 실패, 로그인 필요 상태가 UI로 확인된다
- [ ] 모바일 390px에서 Create/Profile/Requests 주요 텍스트와 버튼이 겹치거나 넘치지 않는다
- [ ] `pnpm --filter @ux-lab/agent-and-my-ax run build`가 통과한다
...
**FE (Avery)**
- [ ] `/agent/new`, `/profile/[userId]`, `/requests` 라우트 구현
- [ ] Create Agent 폼과 등록 후 Feed/Detail 이동 플로우 구현
- [ ] Profile 활동 요약과 Request Board UI 구현
- [ ] 다운로드/resultPreset 입력 UX와 preview 구현

**BE (Blake)**
- [ ] repository interface와 mock adapter 구현
- [ ] `POST /api/agents`, request, interaction API action 정리
- [ ] current user provider와 auth shell 구현
- [ ] Firestore 전환 문서와 보안 규칙 초안 작성
...
**QA (Morgan / Quinn)**
- [ ] Create → Feed → Detail → Run → Tried → Profile → Requests 플로우 테스트
- [ ] 모바일 390px, 데스크톱 1280px overflow 체크
...
| # | 질문 | 담당 | 기한 | 상태 |
|---|------|------|------|------|
| OQ-1 | Sprint 2에서 실제 Firebase 프로젝트를 연결할 수 있는가, 아니면 adapter와 문서까지만 고정할까? | BE Blake | Sprint 2 중 | ⚠️ Open |
| OQ-2 | Agent 등록 시 resultPreset을 작성자가 직접 쓰게 할지, Prompt에서 자동 생성하는 UX로 둘지 결정이 필요하다 | PM Jordan | Sprint 2 중 | ⚠️ Open |
| OQ-3 | Request Board 요청은 전체 공개만 허용할지, 팀 공개 요청도 허용할지 결정이 필요하다 | PM Jordan | Sprint 2 중 | ⚠️ Open |
| OQ-4 | Profile URL은 `/profile/[userId]`와 `/me` 중 무엇을 기본 진입점으로 둘까? | UX Riley | Sprint 2 중 | ⚠️ Open |
| OQ-5 | Fork 생성은 Sprint 2 P1로 둘지, Create Agent의 필수 파생 플로우로 승격할지 결정이 필요하다 | PM Jordan | Sprint 2 중 | ⚠️ Open |
