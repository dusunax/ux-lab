...
- [ ] 올바른 포맷의 CSV 파일을 업로드하면 파싱 오류 없이 문제 목록이 로드된다
- [ ] 포맷 오류가 있는 CSV 업로드 시, 어느 행·필드가 문제인지 명시한 에러 메시지가 표시된다
- [ ] 문제는 매 세션마다 랜덤 순서로 제공된다 (직전 세션과 동일 순서 반복 없음)
- [ ] 보기를 선택하면 정답/오답 여부가 즉시 시각적으로 구분된다 (색상 또는 아이콘)
- [ ] 정답 확인 후 해당 문항의 해설(`explanation`)이 표시된다
- [ ] 현재 문제 번호와 전체 문제 수가 항상 화면에 표시된다
- [ ] 전체 문항 완료 시 세션 결과 요약(정답 수 / 전체 수)이 표시된다
- [ ] 빌드 오류 없이 `vite build` 성공, TypeScript strict 모드 에러 없음
