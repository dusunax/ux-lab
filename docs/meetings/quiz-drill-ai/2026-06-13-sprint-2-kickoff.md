...
- [ ] 오답만 풀기 — 직전 세션 및 학습 이력 오답 목록 정확도 검증
- [ ] 키보드 단축키 — 숫자키 답 선택, Enter/Space 이동, 미선택 시 무시 경계 케이스 검증
- [ ] 배포 URL에서 CSV 업로드 → 세션 완료 전체 플로우 검증
- [ ] TSV 파일 업로드 및 텍스트 붙여넣기 — CSV와 동일 동작 검증 (CSV-EXT-1)
- [ ] 샘플 데이터 2종 로드, CSV 오류 표시, 보기 랜덤화, 효과음 검증
