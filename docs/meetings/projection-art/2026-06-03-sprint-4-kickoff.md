...
| # | 항목 |
|---|------|
| 1 | 키스톤 보정 드래그 UX 완성 — CSS `matrix3d` 기반, 이동/코너 warp/엣지 scale, localStorage 영속 |
| 2 | MediaPipe Pose Web Worker 오프로딩 구현 — 메인 스레드 분리, 좌표 직렬화 최적화 |
| 3 | 시연 프리셋 저장·복원 구현 — Demo 선택 + 키스톤 값 + FPS on/off 저장 가능 |
...
- [x] 키스톤 보정 — 이동/코너 warp/엣지 scale 핸들로 프로젝션 영역 조정 가능, 설정 localStorage 영속
- [x] 키스톤 보정 — 상하 100px·좌우 16px 기본 여백, 캔버스/아웃라인 동일 좌표계, 화면 좌측 하단 조작 설명 박스
- [x] Web Worker — Pose 모델 추론 Worker 분리, Float32Array transferable 전송, 실패 시 메인 스레드 폴백
- [x] Web Worker — Float32Array transferable 직렬화 구현 (33*2 floats, zero-copy)
- [x] 시연 프리셋 — Demo 선택 + 키스톤 값 + FPS on/off localStorage 저장·복원 (`usePreset` 훅)
- [x] 시연 UI — Demo 선택 드롭다운, 프리셋 설정 토글 패널, 키스톤 활성 시 비조작 버튼 흐림 처리
- [x] Demo D — 2손 기본값, 2손/1손 버튼 순서 조정, 권한 모달·손 모드 컨트롤을 키스톤 캔버스 외부로 분리
- [x] Demo A — projection rect 기준 마우스 좌표 보정, 파티클 캔버스 bounds clamp, transparent canvas 배경
- [x] 기존 116 tests 회귀 없이 통과 ← 126 tests · 17 test files
...
**FE (Avery)**
- [x] 키스톤 보정 드래그 UX 구현 — CSS `matrix3d`, 이동/코너 warp/엣지 scale 핸들
- [x] localStorage 영속 로직 구현 — 키스톤 보정값 저장·복원
- [x] 시연 프리셋 저장·복원 UI 구현 — Demo + 키스톤 + FPS 상태 저장, 설정 토글 패널
- [x] Demo 선택 UI 정리 — 버튼 그룹을 드롭다운으로 전환
- [x] Demo A 좌표계 보정 — projection rect 기준 마우스 좌표 및 파티클 bounds clamp
- [x] Demo D UI 정리 — 2손 기본값, 버튼 순서 조정, 권한 모달/손 모드 컨트롤 portal 분리
- [x] Web Worker 오프로딩 연동 — PERF Chase 구현과 인터페이스 맞춤

**PERF (Chase)**
- [x] MediaPipe Pose Web Worker 오프로딩 구현 — Pose 모델 Worker 분리, Float32Array transferable 전송
- [x] Vite dev 환경 Cross-Origin Isolation 헤더 설정 제외 — SharedArrayBuffer 미선택
- [x] Worker 오프로딩 후 메인 스레드 fps 측정 검증

**QA (Morgan / Quinn)**
- [x] `pnpm test` 실행 결과 0 failures 확인 (126 tests, 17 test files)
- [x] 수용 기준 전체 항목 검증 (키스톤 드래그, Web Worker 분리, 프리셋 저장·복원)
