...
- [ ] Chrome 브라우저에서 3종 데모 모두 전체화면으로 실행 가능
- [ ] 마우스 이동·클릭 이벤트가 50ms 이내 시각적으로 반응함 (입력 지연 체감 없음)
- [ ] Demo A: 파티클 1,000개 이상 동시 렌더링 시 프레임 드랍 없이 동작
- [ ] Demo B: Three.js 3D 씬에서 시점 변환이 마우스 위치와 연동되어 자연스럽게 움직임
- [ ] Demo C: Web Audio API로 음원을 실시간 분석하여 비주얼이 주파수 변화에 동기화됨
- [ ] HDMI 프로젝터 연결 상태에서 전체화면 출력 안정적으로 유지
- [ ] 60fps 목표 기준 연속 10분 이상 프레임 드랍 없이 렌더링 유지
- [ ] `pnpm test` (`vitest run`) 실행 시 전체 테스트 0 failures로 통과
- [ ] 커스텀 훅(`useAudioAnalyzer` 등) 단위 테스트 작성 및 통과
- [ ] 3종 데모 컴포넌트 마운트/언마운트 테스트 통과 (Canvas mock 환경)
...
**FE (Avery)**
- [ ] `apps/projection-art/` Vite + React 18 + TypeScript 프로젝트 신설 (pnpm workspace 연동)
- [ ] Vite 개발 서버 구동 및 전체화면(`document.documentElement.requestFullscreen()`) 진입점 구현
- [ ] Demo A: p5.js instance mode → React 컴포넌트 (`<ParticleFlow />`) 구현
- [ ] Demo B: `@react-three/fiber` + `@react-three/drei` 기반 `<NeonTunnel />` 컴포넌트 구현
- [ ] Demo C: `useAudioAnalyzer` 커스텀 훅 + `<AudioReactiveVisual />` 컴포넌트 구현
- [ ] 글로우 효과, 기하학 애니메이션, 색상 팔레트 고도화
- [ ] 데모 전환 라우팅 UI 구현 (React Router 또는 상태 기반 전환)
- [ ] Vitest + @testing-library/react + jsdom 테스트 환경 구성 (`vitest.config.ts`, Canvas/WebGL mock 설정)
- [ ] 커스텀 훅 단위 테스트 작성 (`useAudioAnalyzer` — AudioContext mock, 주파수 데이터 반환 검증)
- [ ] 3종 데모 컴포넌트 마운트/언마운트 테스트 작성 (에러 없이 렌더링, 마우스 이벤트 핸들러 바인딩 확인)
- [ ] 유틸리티 함수 단위 테스트 작성 (파티클 생성 로직, 색상 변환 등)

**PERF (Chase)**
- [ ] requestAnimationFrame 기반 렌더 루프 검토 및 GPU 병목 측정
- [ ] 프레임 드랍 최소화 최적화 작업

**QA (Morgan / Quinn)**
- [ ] `pnpm test` 실행 결과 0 failures 확인 및 테스트 커버리지 리뷰
- [ ] 수용 기준 전체 항목 검증
- [ ] 마우스 이벤트 응답 지연 50ms 기준 검증
- [ ] 60fps 연속 10분 렌더링 유지 검증
- [ ] HDMI 프로젝터 출력 안정성 검증
...
| 질문 | 담당 | 기한 | 상태 |
|------|------|------|------|
| p5.js vs Three.js 데모별 분리 사용 vs 단일 라이브러리 통일 여부 — 유지보수 vs 표현 범위 트레이드오프 | PM + FE Avery | Sprint projection-art/1 킥오프 | ⚠️ Open |
| Demo C 오디오 소스 방식 — 로컬 파일 업로드 / 마이크 입력 / 번들 파일 중 채택 방식 확정 | PM + FE Avery | Sprint projection-art/1 킥오프 | ⚠️ Open |
| 프레임률 목표 수치 — 프로젝터 주사율(60Hz/144Hz) 확인 후 목표 fps 확정 | FE Avery + PERF Chase | Sprint projection-art/1 Day 1 | ⚠️ Open |
