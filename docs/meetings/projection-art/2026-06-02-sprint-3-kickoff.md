...
- [x] Canvas 호모그래피 키스톤 보정 구현 또는 CSS 병행 선택 UI 추가 (OQ-3 결정에 따라)
...
| Canvas 호모그래피 키스톤 보정 도입 범위 — 전면 교체 vs CSS perspective 병행 선택 UI | PERF Chase + FE Avery | Sprint projection-art/3 킥오프 | ✅ 결정: CSS 병행 선택 UI. 기존 matrix3d 유지하며 호모그래피 전환 옵션 추가. |
