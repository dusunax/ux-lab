...
**현재 상태:** 킥오프 시점에서 결정 보류. PM Jordan + FE Avery가 Sprint projection-art/2 킥오프 중 확정 예정.
...
**현재 상태:** 킥오프 시점에서 결정 보류. FE Avery + QA Morgan이 Sprint projection-art/2 킥오프 중 확정 예정.
...
**현재 상태:** 킥오프 시점에서 결정 보류. PERF Chase + FE Avery가 Sprint projection-art/2 킥오프 중 확정 예정.
...
| # | 결정 내용 | 담당 |
|---|-----------|------|
| 1 | Sprint projection-art/2 스코프 7개 항목 확정 | 전원 |
| 2 | 수용 기준 7개 항목 확정 | 전원 |
...
- [ ] Demo D가 Chrome에서 웹캠 권한 승인 후 MediaPipe 모션 추적으로 Three.js 비주얼이 반응함
- [ ] 웹캠 미사용/거부 시 마우스 폴백 모드로 자동 전환됨
- [ ] 인터랙션 어댑터: 마우스 입력과 관절 좌표 입력이 동일 인터페이스로 기존 3종 데모에 적용 가능
- [ ] MediaPipe + WebGL 동시 부하 환경에서 60fps 유지 (연속 5분 이상)
- [ ] `pnpm test` (`vitest run`) 실행 시 전체 테스트 0 failures로 통과
- [ ] `useMotionTracker` 훅 단위 테스트 작성 및 통과 (웹캠 mock 환경)
- [ ] 프로젝션 키스톤 보정 UI가 4코너 드래그로 보정값 저장·적용됨
...
**FE (Avery)**
- [ ] Demo D: MediaPipe Pose/Hands + Three.js 기반 신체 반응형 비주얼 컴포넌트 구현
- [ ] `useMotionTracker` 커스텀 훅 구현 — 웹캠 스트림 초기화 및 관절 좌표 추출
- [ ] 웹캠 권한 요청 UI 구현 + 웹캠 미사용/거부 시 마우스 폴백 자동 전환 로직 구현
- [ ] 인터랙션 어댑터 추상화 — 마우스 입력과 관절 좌표 입력을 동일 인터페이스로 통합, 기존 3종 데모에 적용
- [ ] 프로젝션 키스톤 보정 UI — 4코너 드래그 기반 간이 보정, 보정값 저장·적용 구현
- [ ] `useMotionTracker` 훅 단위 테스트 작성 (웹캠 mock 환경, 관절 좌표 반환 검증)
- [ ] 어댑터 인터페이스 단위 테스트 작성 (마우스 / 관절 좌표 입력 동등성 검증)

**PERF (Chase)**
- [ ] MediaPipe + WebGL 동시 부하 환경에서 60fps 유지 재검증 (연속 5분 이상)
- [ ] MediaPipe 추론 레이턴시 측정 — 메인 스레드 영향 평가 및 Web Worker 오프로딩 설계 검토

**QA (Morgan / Quinn)**
- [ ] `pnpm test` 실행 결과 0 failures 확인 및 테스트 커버리지 리뷰
- [ ] 수용 기준 전체 항목 검증 (Demo D 동작, 폴백 모드, 어댑터 적용)
- [ ] 모션 입력 지연 검증 — MediaPipe 추론 결과가 비주얼 반응에 반영되는 지연 시간 측정
...
| 질문 | 담당 | 기한 | 상태 |
|------|------|------|------|
| MediaPipe Pose vs Hands 모델 선택 — 레이턴시·번들 크기 트레이드오프 | PM + FE Avery | Sprint projection-art/2 킥오프 | ⚠️ Open |
| 인터랙션 어댑터 인터페이스 설계 — 단일 포인트 vs 멀티포인트 배열 | FE Avery + QA Morgan | Sprint projection-art/2 킥오프 | ⚠️ Open |
| 프로젝션 키스톤 보정 수준 — CSS perspective vs Canvas 호모그래피 | PERF Chase + FE Avery | Sprint projection-art/2 킥오프 | ⚠️ Open |
