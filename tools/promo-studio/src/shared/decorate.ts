 ...
 * icons는 쓸 스프라이트 파일명 목록(앱별로 다름)이고, 크기(46~136px)와 위치, 회전, 등장 지연이 전부 랜덤이다. exclude로 준 영역(스크린샷·자막 박스)은
 ...
 */
export function randomDecorations(icons: readonly string[], seed: string, count: number, exclude: Zone[], yRange: [number, number] = [SAFE_TOP, SAFE_BOTTOM_Y]): Deco[] {
  const decos: Deco[] = []
  ...
  for (let i = 0; i < count; i++) {
    ...
    decos.push({
      name: icons[Math.floor(random(`${seed}-${i}-icon`) * icons.length)],
      x,
      y,
      size,
